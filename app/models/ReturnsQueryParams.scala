...
case class ReturnsQueryParams(from: LocalDate, to: LocalDate, status: String) {
  val map = Map("from" -> from, "to" -> to, "status" -> status)
}

object ReturnsQueryParams {
  val dateRegex = """^\d{4}-\d{2}-\d{2}$"""
  val statusRegex = "^[OFA]$"


  def from(fromOpt: OptEither[String], toOpt: OptEither[String], statusOpt: OptEither[String]): Either[String, ReturnsQueryParams] = {
    ...
    if (errors.isEmpty) {
      Right(ReturnsQueryParams(from.map(_.right.get).get, to.map(_.right.get).get, status.map(_.right.get).get))
    } else {
      Left(errors.head)
    }
  }
  ...
  private def validDateRange(fromOpt: OptEither[LocalDate], toOpt: OptEither[LocalDate]) = {
    ...
    for {
      ...
      (fromVal.right.get, toVal.right.get) match {
        case (from, to) if !from.isBefore(to) || from.plusDays(oneYear).isBefore(to) =>
